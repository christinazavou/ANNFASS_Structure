...
_C.DATA.train.normalise = False  # Apply ocnn normalisation to points
...
_C.MODEL.feature_threshold = 2 ** 100  # HRNet maximum feature map size (reduce memory & time), if 0 no clipping is applied
...
def parse_args(backup=True):
    ...
    assert os.path.exists(FLAGS.SOLVER.ckpt+".index") or FLAGS.SOLVER.ckpt == '', "Path of provided checkpoint does " \
                                                                                  "not exist "
    return FLAGS
