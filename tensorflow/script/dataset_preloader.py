...
class DataLoader:
    ...
    def __call__(self, flags, nout, channels, mem_check=False, test_phase=False):
        ...
        def read_files(filenames, nout, channels):
            ...
            for fname in filenames: #tqdm(filenames, leave=False, file=sys.stdout):
                ...
                try:
                    ...
                    print("Could not open label file. Exiting...")
                    sys.exit()
        ...
        if self.flags.take != -1:
            if self.flags.take >= self.tfrecord_num:
                ...
                warn(
                    "Flag take is not -1. Keeping {} records from file".format(
                        self.flags.take))
                del self.filenames[:self.flags.take]
